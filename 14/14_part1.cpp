...
bool recur(vector<vector<int>>& vvb, ll int stx, ll int sty, ll int target) {    
    // cout << stx << " " << sty << endl;
    if(stx > target) return true;
    
    if(vvb[stx + 1][sty] == 0) {
        return recur(vvb, stx + 1, sty, target);
    } else if(vvb[stx + 1][sty - 1] == 0) {
        return recur(vvb, stx + 1, sty - 1, target);
    } else if(vvb[stx + 1][sty + 1] == 0) {
        return recur(vvb, stx + 1, sty + 1, target);
    } else {
        // no possible play
